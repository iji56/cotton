...
import { faLocationCrosshairs, faLocationDot, faPencil } from '@fortawesome/pro-light-svg-icons'
...
const AddressesNewSignIn = () => {
    ...
    const [isEdit, setIsEdit] = useState<{ address: string } | string>({ address: '' });
    ...
    useEffect(() => {
        const fetchSavedAdresses = async () => {
            ...
            if (sessionData?.uid) {
                let addresses = await getSavedAddresses(sessionData.uid);
                if (addresses) {
    ...
    const handleContinue = () => {
        if (address) {
            navigation.navigate('AddressMapNewSignIn', { address, sessionData, isEdit })
        } else {
    ...
    const handleEdit = (address: any, nearby?: boolean) => {
        if (!nearby) {
            setIsEdit(address)
        } else {
            setIsEdit({ address: "" })
    ...
    const renderItem = (address: { address: string }, index: number, nearby?: boolean) => {
        ...
        return (
            <TouchableOpacity key={index} style={SAA.listItemContainer} onPress={() => handleEdit(address, nearby)}>
                <FontAwesomeIcon icon={nearby ? faLocationCrosshairs : faLocationDot} size={20} style={SAA.icon} />
                <View style={{ flex: 1 }}>
                    <Text>{mainText}</Text>
                ...
                {!nearby &&
                    <IconButton
                        onPress={() => handleEdit(address, nearby)}
                        icon={faPencil}
                        style={SAA.editIcon}
                    />
                }
        ...
        <View style={{
            ...
            <ScrollView
                ...
                <View style={SAA.listContainer}>
                    ...
                    />
                    <H2 text={keywords.savedAddresses} style={SAA.addressText} />
                    <FlatList
                        data={addresses}
                        keyExtractor={(item, index) => index.toString()}
            ...
            {!inputFocused &&
                <View style={SAA.button}>
                    <Button text={keywords.continue} onPress={handleContinue} variant='main' />
                </View>
            }
        </View>
    )
...
export default AddressesNewSignIn