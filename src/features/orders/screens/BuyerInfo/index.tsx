...
import SBI from './BuyerInfo.styles';
...
import { updateSellerAddress } from '../../api/updateSellerAddress';
import Geocoder from 'react-native-geocoding';
import { getListingAddress } from '../../api/getListingAddress';
import { cancelOrderByBuyer } from '../../api/cancelOrderByBuyer';
import { acceptOrderBySeller } from '../../api/acceptOrderBySeller';
import { getListingUpdateAddressStatus } from '../../api/getListingUpdateAddressStatus';
...
import { updateListingPurchaseStatus } from '../../api/updateListingPurchaseStatus';
...
import { sendOnAcceptBuyLocalPickupEmails, sendOnAcceptBuyShippingEmails } from '../../api/sendOrderEmails';
import { getShipmentArtifect } from '../../api/getArtifect';

const BuyerInfo = ({ route }: any) => {
    ...
    const [updateAddressSnapPoint, setUpdateAddressSnapPoint] = useState<number | string>('1%');
    const [addressUpdatedSnapPoint, setAddressUpdatedSnapPoint] = useState<number | string>('1%');
    ...
    const getUserLocation = () => {
        GetLocation.getCurrentPosition({
            ...
            .then(async location => {
                ...
                // stopInterval();
                const parseLocation = await Geocoder.from(address.address.split("#")[0]);
    ...
    const getBuyerDetail = async () => {
        try {
            ...
            const receiverDetail = await getUsermeta(listingDetail?.purchaser_id);
            ...
            await getListingUpdateAddressStatus(listingDetail?.listings_purchased_id)
    ...
    useEffect(() => {
        ...
        const startDate = new Date((listing?.cp_id ? listing?.shipping_date : listing?.local_pickup_date) || null)
        const endDate = new Date((listing?.cp_id ? listing?.shipping_date : listing?.local_pickup_date) || null)
    ...
    const handleAccept = async () => {
        try {
            ...
            if (listing?.cp_id) {
                        ...
                        createShipment(createShipmentPayloadSample.shippingPoint, senderAddress, destinationAddress, createShipmentPayloadSample.parcel).then((response) => {
                            if (response?.error || response?.status === false || response?.response?.error || response?.response?.status === false) {
                                ...
                                if (response?.response) {
                                    response?.response && acceptOrderBySeller(listing?.listings_purchased_id, listing?.purchaser_id, [response.response]).
                                        then(async () => {
                                            const shippingArtifact = await getShipmentArtifect({shippingId: response?.response.labelId})
                                            
                                            await sendOnAcceptBuyShippingEmails(listing, user, buyer.user_info,keywords.shipping, shippingArtifact)
                                            // send message
                                            await createChatAndNavigate(user.id!, buyer.user_info.id, buyer.user_info.user_name, navigation, note || "undefined");
                                            navigation.goBack();
                                            setIsLoading(false);
                ...
                await confirmPaymentIntent(listing?.payment_client_secret, listing.payment_method_id).then(() => {
                    updateListingPurchaseStatus(listing?.listings_purchased_id, listing?.purchaser_id).
                        then(async () => {
                            await sendOnAcceptBuyLocalPickupEmails(listing, user, buyer.user_info, keywords.localPickup)
                            // send message
                            await createChatAndNavigate(user.id!, buyer.user_info.id, buyer.user_info.user_name, navigation, note || "undefined");
                            navigation.goBack();
                            setIsLoading(false);
    ...
    const handleDeny = () => {
        cancelOrderByBuyer(listing?.listings_purchased_id, 0, listing?.purchaser_id).then(async () => {
            // send message
            await createChatAndNavigate(user.id!, buyer.user_info.id, buyer.user_info.user_name, navigation, note || "undefined");
            navigation.goBack();
        }).catch(error => {
    ...
    const updateAddress = async () => {
        ...
        await updateSellerAddress(addressDetail[0]?.address, location?.lat || 0, location?.lng || 0, user.id!, listing.id, listing?.purchaser_id, listing?.listings_purchased_id, addressDetail[0].postal_code);
    }

    const handleChat = async () => {
        setLoading(true)
        await createChatAndNavigate(user.id!, buyer.user_info.id, buyer.user_info.user_name!, navigation);
        setLoading(false)
    }
            ...
            {!isLoading ? <>
                ...
                <Wrapper>
                    <ScrollView contentContainerStyle={{ paddingBottom: 200 }} showsVerticalScrollIndicator={false} keyboardDismissMode='on-drag'>
                        ...
                        <TouchableOpacity style={SBI.userContainer} onPress={() => navigation.navigate('FeedProfile', { userID: buyer.user_info.id })}>
                            <Image source={{ uri: buyer?.user_info?.userAvatar || picture }} style={SBI.userImage} />
                        ...
                        <View style={SBI.listingContainer}>
                            <ListingItem itemData={{ image: listing?.image_url || picture, name: listing?.listing_name, type: listing?.brand, size: listing?.size }}
                                selectedDates={listing?.cp_id ? [listing?.shipping_date, listing?.shipping_date] :
                                    [listing?.local_pickup_date, listing?.local_pickup_date]
                                } />
                        </View>

                        <H2 text={keywords.buyDate} style={SBI.subHeading} />
                        <Text style={SBI.text}>{!listing.cp_id ? keywords.meetupOn : keywords.shipOn} {start}</Text>

                        <H2 text={keywords.buyMethod} style={SBI.subHeading} />
                        ...
                        <Text style={SBI.text}>{address?.address?.split("#")[0] || 'address'}</Text>
                        ...
                        <Text style={SBI.text}>{'CA $'}{(listing?.price_sale * 0.8)?.toFixed(2) ?? 0}</Text>
                        {listing?.message &&
                            <View style={SBI.userSmallContainer}>
                                <View>
                                    <H2 text={keywords.messageFromBuyer} />
                                    <Text style={SBI.message}>{listing?.message}</Text>
                                    <Button text={keywords.reply} onPress={handleChat} variant='main' style={{ width: 80 }} />
                                </View>
                            </View>
                        }
                        <TouchableOpacity style={[SBI.userSmallContainer, { marginTop: 0 }]} onPress={() => navigation.navigate('FeedProfile', { userID: buyer.user_info.id })}>
                            <Avatar size='s' avatar={buyer?.user_info?.userAvatar || picture} />
                ...
                <View style={SBI.bottomContainer}>
                    {addressUpdated ?
                        <View style={SBI.disableButtonContainer}>
                            <Button text={keywords.waitingForBuyerToReview} onPress={() => { }} variant='secondary' style={SBI.disableButton} buttonTextColor={palette.white} fontSize={14} />
                        </View> :
