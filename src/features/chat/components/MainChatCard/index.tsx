...
const MainChatCard = ({ chatID, name, message, elapsedTime, picture, last, sender, status, duration, read }: ChatCardProps) => {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('ChatSingle', { chatID, receiverName, picture })}
    >
      <Wrapper>
        <View style={SMCC.mainContainer}>
          <View style={SMCC.avatarContainer}>
            <Avatar avatar={picture?.startsWith('http') ? picture : `${baseUrl}${picture}`} size='s' />
          </View>
          <View style={SMCC.content}>
            ...
            <Text style={[SMCC.meta, { fontWeight: (read || sender) ? '400' : '600', width: width * 0.7 }]} numberOfLines={2} ellipsizeMode='tail'>
              {messagePreview}
            </Text>
